...
const ManageProduct = ({ products, fetchProducts }) => {
  ...
  const handleSaveClick = async () => {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/productsadd/update/${
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/productsadd/delete/${
  ...
  const toggleActiveState = async (product) => {
    ...
    try {
      const response = await axios.put(
        ...
        {
          activeState: product.activeState === "Active" ? "Disable" : "Active",
        }
      );
      ...
      <div className="flex justify-center items-center gap-2 mt-4">
        <button
          ...
          className={`px-3 py-1 mx-1 rounded text-lg font-medium ${
            currentPage === totalPages ? " text-[black]" : " text-[#777777]"
          }`}
