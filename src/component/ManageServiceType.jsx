...
const ManageServiceType = ({ services, fetchServices }) => {
  ...
  const handleEditClick = (service) => {
    ...
    setEditedServiceType(service.servicetype);
  };
  ...
  const handleSaveClick = async () => {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/servicetype/update/${
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/servicetype/delete/${
  ...
  const toggleActiveState = async (service) => {
    setLoading(true);
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/servicetype/update/${
        ...
        {
          activeState: service.activeState === "Active" ? "Disable" : "Active",
        }
      );
      ...
      <div className="lg:w-[90%] sm:w-full">
        <div className="flex flex-col gap-2">
          <div className="flex flex-row p-4 gap-4 font-medium text-base bg-black text-white w-full border-b">
            <div className="w-[40%]">Service Type</div>
